...
export function computeclosestcoordsfromevent( event: mapboxgl.MapMouseEvent & {
    features?: mapboxgl.MapboxGeoJSONFeature[] | undefined;
} & mapboxgl.EventData ): number[] {
    if (event.features) {
        ...
        setofuniquepoints.forEach((stringedcoordinates:string) => {
            const to = turf.point(stringedcoordinates.split(',').map((coord) => parseFloat(coord)));
            const distance = turf.distance(from, to, options);
            distancesofuniquepoints[stringedcoordinates] = distance;
        });

        let shortestdistanceset = Object.keys(distancesofuniquepoints).reduce((key, v) => distancesofuniquepoints[v] < distancesofuniquepoints[key] ? v : key);

        
        var result =  shortestdistanceset.split(',').map((coord) => parseFloat(coord));
